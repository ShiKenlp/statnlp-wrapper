...
GlobalNetworkParam::GlobalNetworkParam(int vocab_size) {
    is_locked_ = false;
    h_feature_size_ = 0;
...
bool GlobalNetworkParam::UpdateDiscriminative() {
    ...
    if(ComParam::OPT_LBFGS == ComParam::OPTIMIZER){
        go_on_training = this->ptr_opt_->optimize(h_feature_size_,ptr_weights_,-obj_current_,ptr_counts_, false, 0.0);
