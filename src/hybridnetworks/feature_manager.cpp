...
FeatureArray* FeatureManager::Extract(Network *ptr_network, int parent_k, int *ptr_children_k, int children_k_index) {
    ...
    // add for python wrapper

//    std::cout<<"before featureArray ExtractHelper."<<std::endl;
    int csize =0;
    csize = ptr_network->GetChildren_Size(parent_k)[children_k_index];
    //std::cout<<"parent node: "<< parent_k<<" the children size: "<<csize<<std::endl;
    std::vector<int> children_vec;
    for (int i =0; i < csize; ++i){
        children_vec.push_back(ptr_children_k[i]);
        //std::cout<<ptr_children_k[i]<<std::endl;
    }
    //FeatureArray* ptr_fa = this->ExtractHelper(ptr_network,parent_k,ptr_children_k);
    FeatureArray* ptr_fa = this->ExtractHelper(ptr_network,parent_k,children_vec);
    //std::cout<<"After featureArray ExtractHelper."<<std::endl;
    //std::cout<<"In the featureArray Extract. Focus on the return value."<<std::endl;
    //ptr_fa->GetFeatureArray();
    if(this->isCacheAble()){
...
//FeatureArray* FeatureManager::ExtractHelper(Network *ptr_network, int parent, int *ptr_children) {
//
//
//}
