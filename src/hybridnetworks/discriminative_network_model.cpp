...
// std::cout <<"discriminativemodel is called"<<std::endl;
}
...
//    std::cout <<"the size of intance is "<<pptr_insts_->size()<<std::endl;
    int threadId = 0;
    ...
    for(threadId = 0; threadId < this->num_threads_; ++threadId){
        ...
        pptr_insts_->push_back(ptr_vec);  // why push an empty ptr_vec?
...
//    std::cout<< "split function:"<< this->ptr_inst_all_->size()<<std::endl;
    return pptr_insts_;
}
