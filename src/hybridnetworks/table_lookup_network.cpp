...
TableLookupNetwork::TableLookupNetwork() {
    ...
    //added later
    //ptr_children_vec_ = nullptr;
    //ptr_children_size_vec_ = nullptr;
}
...
TableLookupNetwork::TableLookupNetwork(int networkId, Instance *ptr_inst, LocalNetworkParam *ptr_param):Network(networkId,ptr_inst,ptr_param) {
    //std::cout<< "table lookup"<<std::endl;
    ptr_children_tmp_ = new std::unordered_map<long, std::list<std::vector<long>*>*>;
    ptr_childrens_size_ = nullptr;
    ptr_children_size_ = nullptr;
    ptr_nodes_ = nullptr;
    ...
    //added later for python wrapper
    //ptr_children_vec_ = nullptr;
    //ptr_children_size_vec_ = nullptr;
}
...
TableLookupNetwork::TableLookupNetwork(int networkId, Instance *ptr_inst, TableLookupNetwork *ptr_network,
                                       LocalNetworkParam *ptr_param, int num_nodes):Network(networkId,ptr_inst,ptr_param) {
    //std::cout<<"new constructor is called."<<std::endl;
    ptr_children_tmp_ = new std::unordered_map<long, std::list<std::vector<long>*>*>;
    node_size_ = num_nodes;
    ptr_nodes_ = ptr_network->GetAllNodes();
    //std::cout<<"the ptr_nodes"<< ptr_nodes_[0]<<","<<ptr_nodes_[1]<<","<<ptr_nodes_[2]<<std::endl;
    ptr_children_ = ptr_network->GetAllChildren();
    ptr_children_size_ = ptr_network->GetChildren_Size();
    //std::cout<<"**********************In the TLN, the CHILDREN size is:"<< ptr_children_size_[1][0]<<std::endl;
    ptr_childrens_size_ = ptr_network->GetChildrens_Size();
    //std::cout<<"the ptr_nodes"<< ptr_childrens_size_[0]<<","<<ptr_childrens_size_[1]<<","<<ptr_childrens_size_[2]<<std::endl;
}

bool TableLookupNetwork::AddNode(long nodeId) {
    //std::cout<< "add node in c++"<< std::endl;
...
long * TableLookupNetwork::GetAllNodes() {
    //std::cout<<"Before return the long * in GetAllNodes."<<std::endl;
    return this->ptr_nodes_;
}

//std::vector<long> TableLookupNetwork::GetNodesAll(){
//    for (int i=0; i<node_size_; i++){
//        vec_nodes_.push_back(ptr_nodes_[i]);
//    };
//}
...
void TableLookupNetwork::FinalizeNetwork() {
    //added later for python wrapper
 //   ptr_children_vec_ =  new std::vector<std::vector<std::vector<int>>>;
//    ptr_children_size_vec_ = new std::vector<std::vector<int>>;
//    ptr_childrens_size_vec_ = new std::vector<int>;
    ...
    for(auto it = this->ptr_children_tmp_->begin(); it!=this->ptr_children_tmp_->end(); ++it){
        ...
        if(nullptr == ptr_childrens){
            ...
            this->ptr_children_[parent_index][0] = new int[0];
            //this->ptr_children_[parent_index][0][0] = nullptr;
            this->ptr_childrens_size_[parent_index] = 1;
            this->ptr_children_size_[parent_index] = new int[1];
            this->ptr_children_size_[parent_index][0] = 0;
            //added later for python wrapper
//            std::vector<std::vector<int>> children_vec_vec;
//            std::vector<int> vec{0};
//            children_vec_vec.push_back(vec);
//            ptr_children_vec_->push_back(children_vec_vec);
        }else{
            ...
            for(auto itt = ptr_childrens->begin(); itt!=ptr_childrens->end(); ++itt){
                ...
                for(auto ittt = (*itt)->begin(); ittt != (*itt)->end(); ++ittt){
                    ...
                    //vec.push_back(ptr_nodeId2Index_map_tmp->find((*ittt))->second);
                    node_no++;
                }
...
int TableLookupNetwork::CountNodes() {
    //std::cout<<"CountNodes in C++ TLN"<<node_size_<<std::endl;
    return node_size_;
}
