...
//        ptr_nn_g_->SetLearningState();
    ...
    std::vector<std::vector<Instance*>*> *ptr_inst = this->SplitInstanceForTrain();  // what does *** mean.
//    std::cout <<"before touch"<<std::endl;
    for (int threadId = 0; threadId < this->num_threads_; ++threadId) {
...
//    std::cout <<"after toutch"<<std::endl;
    ...
    std::cout <<"The number of Feature: "<<this->ptr_fm_->GetGlobalParam()->tmp_count_<<std::endl;
    //std::cout <<"after tem_count_"<<std::endl;
